...
function checkBuild() {
  ...
  if (!fs.existsSync('dist/foundation-x.css')) {
    errors.push(
      '! Missing "foundation-x.css": be sure to run "build:css" after "build:pkg"',
    );
  }
  ...
  if (
    ...
    errors.push(
      '! Missing "index.cjs": be sure to run "npm run build" or "npm run build:pkg"',
    );
  }

  if (!fs.existsSync('dist/index.d.cts')) {
    errors.push(
      '! Missing types "index.d.cts": be sure to run "npm run build" or "npm run build:pkg"',
    );
  }

  if (!fs.existsSync('dist/index.js') || !fs.existsSync('dist/index.js.map')) {
    errors.push(
      '! Missing "index.js": be sure to run "npm run build" or "npm run build:pkg"',
    );
  }

  if (!fs.existsSync('dist/index.d.ts')) {
    errors.push(
      '! Missing types "index.d.ts": be sure to run "npm run build" or "npm run build:pkg"',
    );
  }
